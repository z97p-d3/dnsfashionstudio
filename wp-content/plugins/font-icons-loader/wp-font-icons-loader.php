...
Version: 2.1 Kaswara mod 23.05.2018
Author: Templines
...
function pixfil_fonts_activate(){
	...
	$fil_upload_dir = wp_upload_dir();
    $fil_dir = $fil_upload_dir['basedir'].'/kaswara/fonts_icon/';
...
function fil_enqueue_styles() {
    ...
    $ksw_fonts = class_exists( 'Kaswara_Icon_Manager' ) ? Kaswara_Icon_Manager::get_font_icons() : array();
    
    if(!empty($active_fonts)){
        
        foreach($active_fonts as $key => $val){
            if( file_exists($fil_dir . $val) && $val != '' ){
                if (!in_array($key, $ksw_fonts)){
